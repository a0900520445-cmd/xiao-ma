...
router.post('/announcements', requireAuth, async (req, res) => {
  ...
  try {
    ...
    // 發布新公告 → 寫入前台通知
    await pool.query(
      `INSERT INTO notifications (type,title,message,link,target) VALUES ($1,$2,$3,$4,'public')`,
      ['announcement', '📢 新公告', title, '/#announcements']
    )
    // 推送 Web Push 通知給已訂閱的訪客
    await req.app.locals.sendPushToTarget('public', {
      title: '📢 御園國小 新公告',
      body: title,
      url: '/#announcements'
    })
...
// 批次更新排序（拖拽排序用）：body = { order: [{id, sort_order}, ...] }
router.put('/nav-buttons-reorder', requireAuth, async (req, res) => {
  const { order } = req.body
  if (!Array.isArray(order)) return res.json({ success: false, message: '格式錯誤' })
  const pool = req.app.locals.pool
  try {
    for (const item of order) {
      await pool.query('UPDATE nav_buttons SET sort_order=$1 WHERE id=$2', [item.sort_order, item.id])
    }
    res.json({ success: true })
  } catch (err) { res.status(500).json({ success: false, error: err.message }) }
})
...
// ── 榮譽榜 CRUD（後台）────────────────────────────────────
router.get('/honors', requireAuth, async (req, res) => {
  const pool = req.app.locals.pool
  try {
    const r = await pool.query('SELECT * FROM honor_board ORDER BY award_date DESC, created_at DESC')
    res.json({ success: true, data: r.rows })
  } catch (err) { res.status(500).json({ success: false, error: err.message }) }
})

router.post('/honors', requireAuth, async (req, res) => {
  const { title, category, student_name, class_name, award, description, award_date } = req.body
  if (!title || !category || !student_name || !class_name || !award)
    return res.json({ success: false, message: '請填寫所有必填欄位' })
  const pool = req.app.locals.pool
  try {
    const r = await pool.query(
      `INSERT INTO honor_board (title,category,student_name,class_name,award,description,award_date)
       VALUES ($1,$2,$3,$4,$5,$6,$7) RETURNING id`,
      [title, category, student_name, class_name, award, description||null, award_date||null]
    )
    // 新榮譽記錄 → 寫入前台通知
    await pool.query(
      `INSERT INTO notifications (type,title,message,link,target) VALUES ($1,$2,$3,$4,'public')`,
      ['honor', '🏆 榮譽榜更新', `${student_name}（${class_name}）獲得「${title}」`, '/honor_board']
    )
    await req.app.locals.sendPushToTarget('public', {
      title: '🏆 榮譽榜更新',
      body: `${student_name}（${class_name}）獲得「${title}」`,
      url: '/honor_board'
    })
    res.json({ success: true, id: r.rows[0].id })
  } catch (err) { res.status(500).json({ success: false, error: err.message }) }
...
router.put('/honors/:id', requireAuth, async (req, res) => {
  const { title, category, student_name, class_name, award, description, award_date } = req.body
  const pool = req.app.locals.pool
  try {
    await pool.query(
      `UPDATE honor_board SET title=$1,category=$2,student_name=$3,class_name=$4,award=$5,description=$6,award_date=$7 WHERE id=$8`,
      [title, category, student_name, class_name, award, description||null, award_date||null, req.params.id]
    )
    res.json({ success: true })
  } catch (err) { res.status(500).json({ success: false, error: err.message }) }
})

router.delete('/honors/:id', requireAuth, async (req, res) => {
  const pool = req.app.locals.pool
  try {
    await pool.query('DELETE FROM honor_board WHERE id=$1', [req.params.id])
    res.json({ success: true })
  } catch (err) { res.status(500).json({ success: false, error: err.message }) }
})
...
router.get('/notifications', requireAuth, async (req, res) => {
  ...
  try {
    const r = await pool.query(
      `SELECT * FROM notifications WHERE target='admin' ORDER BY created_at DESC LIMIT 30`
    )
    const unreadR = await pool.query(
      `SELECT COUNT(*) as cnt FROM notifications WHERE target='admin' AND is_read=FALSE`
    )
    res.json({ success: true, data: r.rows, unread: parseInt(unreadR.rows[0].cnt) })
  } catch (err) { res.status(500).json({ success: false, error: err.message }) }
})
...
router.put('/notifications/read-all', requireAuth, async (req, res) => {
  const pool = req.app.locals.pool
  try {
    await pool.query(`UPDATE notifications SET is_read=TRUE WHERE target='admin' AND is_read=FALSE`)
    res.json({ success: true })
  } catch (err) { res.status(500).json({ success: false, error: err.message }) }
})
...
// 新增加點記錄（若學生不存在則自動建立學生資料）
router.post('/points', requireAuth, async (req, res) => {
  const { student_id, name, class_name, points, reason } = req.body
  if (!student_id || !name || !class_name || points === undefined || points === null || points === '')
    return res.json({ success: false, message: '請填寫所有必填欄位' })
  const pointsNum = parseInt(points)
  if (isNaN(pointsNum)) return res.json({ success: false, message: '點數必須是數字' })

  const pool = req.app.locals.pool
  try {
    // 確保學生資料存在（不存在就新增，存在則更新姓名/班級）
    await pool.query(
      `INSERT INTO students (student_id, name, class_name) VALUES ($1,$2,$3)
       ON CONFLICT (student_id) DO UPDATE SET name=$2, class_name=$3`,
      [student_id, name, class_name]
    )
    // 寫入加點記錄
    const r = await pool.query(
      `INSERT INTO point_records (student_id, points, reason) VALUES ($1,$2,$3) RETURNING id`,
      [student_id, pointsNum, reason || null]
    )
    res.json({ success: true, id: r.rows[0].id })
  } catch (err) { res.status(500).json({ success: false, error: err.message }) }
})
...
// 取得單一學生的所有點數記錄（後台檢視用）
router.get('/points/:student_id', requireAuth, async (req, res) => {
  const pool = req.app.locals.pool
  try {
    const r = await pool.query(
...
// 刪除單筆加點記錄
router.delete('/points/:id', requireAuth, async (req, res) => {
  const pool = req.app.locals.pool
  try {
    await pool.query('DELETE FROM point_records WHERE id=$1', [req.params.id])
    res.json({ success: true })
  } catch (err) { res.status(500).json({ success: false, error: err.message }) }
})

// 編輯單筆加點記錄
router.put('/points/:id', requireAuth, async (req, res) => {
  const { points, reason } = req.body
  if (points === undefined || points === null || points === '')
    return res.json({ success: false, message: '請輸入點數' })
  const pointsNum = parseInt(points)
  if (isNaN(pointsNum)) return res.json({ success: false, message: '點數必須是數字' })
  const pool = req.app.locals.pool
  try {
    await pool.query(
      'UPDATE point_records SET points=$1, reason=$2 WHERE id=$3',
      [pointsNum, reason || null, req.params.id]
    )
    res.json({ success: true })
  } catch (err) { res.status(500).json({ success: false, error: err.message }) }
})
...
// 刪除學生（連同其所有點數記錄）
router.delete('/students/:student_id', requireAuth, async (req, res) => {
  const pool = req.app.locals.pool
  try {
    await pool.query('DELETE FROM point_records WHERE student_id=$1', [req.params.student_id])
    await pool.query('DELETE FROM students WHERE student_id=$1', [req.params.student_id])
    res.json({ success: true })
  } catch (err) { res.status(500).json({ success: false, error: err.message }) }
})
