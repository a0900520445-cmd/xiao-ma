...
// ── Web Push VAPID 設定 ──────────────────────────────────
// 正式環境請用環境變數 VAPID_PUBLIC_KEY / VAPID_PRIVATE_KEY
// 沒設定時使用以下預設值（僅供測試，正式上線請自行產生新的一組：
// node -e "console.log(require('web-push').generateVAPIDKeys())"）
const VAPID_PUBLIC_KEY  = process.env.VAPID_PUBLIC_KEY  || 'BJSPNKknAKyK2HBW_Qy32KFeChvpyJNE2_la72W2E67nY7ir4oPeCmC0iNeoO77dU9yzVkwgWqDX3NtWn01K1cg'
const VAPID_PRIVATE_KEY = process.env.VAPID_PRIVATE_KEY || 'xM3d5HVi1jU0AQWpOS9qQn2-q5o_49BbT2hXwskq5DE'
webpush.setVapidDetails(
  'mailto:admin@yuanpark.edu.tw',
  VAPID_PUBLIC_KEY,
  VAPID_PRIVATE_KEY
)
app.locals.webpush = webpush
app.locals.vapidPublicKey = VAPID_PUBLIC_KEY

// 共用：推送通知給指定 target（public / admin）的所有訂閱者
app.locals.sendPushToTarget = async function(target, payload) {
  try {
    const r = await pool.query('SELECT * FROM push_subscriptions WHERE target=$1', [target])
    const results = await Promise.allSettled(r.rows.map(sub => {
      const pushSub = {
    ...
    // 清除失效的訂閱（410 Gone / 404）
    for (let i = 0; i < results.length; i++) {
      if (results[i].status === 'rejected') {
        const err = results[i].reason
        if (err && (err.statusCode === 410 || err.statusCode === 404)) {
          await pool.query('DELETE FROM push_subscriptions WHERE endpoint=$1', [r.rows[i].endpoint])
        }
      }
    }
  } catch (err) {
...
// 開始一筆新錄影紀錄
app.post('/admin-api/recording/start', (req, res) => {
  if (!req.session || !req.session.userId) return res.status(401).json({ success: false })
  const filename = `live-${Date.now()}.webm`
  const filePath = path.join(RECORDINGS_DIR, filename)
  fs.writeFileSync(filePath, Buffer.alloc(0)) // 建立空檔，之後逐段附加
  pool.query(
    `INSERT INTO live_recordings (title, filename, status) VALUES ($1,$2,'recording') RETURNING id`,
    [req.body.title || '直播錄影', filename]
  ).then(r => {
    res.json({ success: true, id: r.rows[0].id, filename })
  }).catch(err => res.status(500).json({ success: false, error: err.message }))
})

// 接收錄影片段，附加寫入對應檔案
app.post('/admin-api/recording/chunk', upload.single('chunk'), (req, res) => {
  if (!req.session || !req.session.userId) return res.status(401).json({ success: false })
  const { filename } = req.body
  if (!filename || !req.file) return res.status(400).json({ success: false, message: '缺少檔案或檔名' })
  // 防止路徑跳脫
  const safeName = path.basename(filename)
  const filePath = path.join(RECORDINGS_DIR, safeName)
  if (!filePath.startsWith(RECORDINGS_DIR)) return res.status(400).json({ success: false })
  try {
...
// 結束錄影
app.post('/admin-api/recording/stop', async (req, res) => {
  if (!req.session || !req.session.userId) return res.status(401).json({ success: false })
  const { id } = req.body
  try {
    await pool.query(
      `UPDATE live_recordings SET status='done', ended_at=NOW() WHERE id=$1`,
      [id]
    )
    res.json({ success: true })
  } catch (err) {
...
// 取得最新一筆已完成的錄影（觀眾端回放用，公開）
app.get('/api/recording/latest', async (req, res) => {
  try {
    const r = await pool.query(
      `SELECT * FROM live_recordings WHERE status='done' ORDER BY started_at DESC LIMIT 1`
    )
...
// 取得所有已完成的錄影列表（觀眾端回放用，公開）
app.get('/api/recording/list', async (req, res) => {
  try {
    const r = await pool.query(
      `SELECT * FROM live_recordings WHERE status='done' ORDER BY started_at DESC LIMIT 30`
    )
...
app.get('/liveadmin', (req, res) => {
  if (!req.session || !req.session.userId) return res.redirect('/')
  res.sendFile(path.join(__dirname, 'public', 'liveadmin.html'))
})
...
async function initDB() {
  try {
    ...
    for (const [k,v] of [
      ...
      ['site_email','info@yuanpark.edu.tw'],['visitor_count','0']
    ]) {
    ...
    if (parseInt(nc.rows[0].cnt) === 0) {
      for (const [name,url,pid,ord,tgt] of [
        ...
        ['最新消息','/news.html',null,2,'_self'],['線上直播','/live',null,3,'_self'],
        ['聯絡我們','/contact',null,4,'_self'],
      ]) {
    ...
    await pool.query(`CREATE TABLE IF NOT EXISTS point_records (
      id BIGSERIAL PRIMARY KEY,
    ...
    await pool.query(`CREATE TABLE IF NOT EXISTS live_recordings (
      id BIGSERIAL PRIMARY KEY,
      title TEXT,
...
// ── 直播訊號伺服器（WebRTC Signaling）──────────────────────
// 廣播端（liveadmin.html）與觀眾端（live.html）透過這裡交換 WebRTC 連線資訊
// 採用一對多廣播模式：廣播端對每個觀眾各自建立一條 P2P 連線
const wss = new WebSocketServer({ server, path: '/live-ws' })

let broadcasterSocket = null          // 目前的廣播端連線（同時只允許一個）
...
wss.on('connection', (ws) => {
  ...
  ws.on('message', (raw) => {
    ...
    // 廣播端宣告身份，開始直播
    if (msg.type === 'broadcaster-join') {
      ws.role = 'broadcaster'
      broadcasterSocket = ws
      // 通知所有已連線的觀眾：直播已開始
      viewerSockets.forEach((vws) => {
        if (vws.readyState === vws.OPEN) {
          vws.send(JSON.stringify({ type: 'live-status', live: true }))
        }
      })
      return
    }
    ...
    if (msg.type === 'viewer-join') {
      ws.role = 'viewer'
      viewerSockets.set(ws.id, ws)
      ws.send(JSON.stringify({ type: 'live-status', live: !!broadcasterSocket }))
      if (broadcasterSocket && broadcasterSocket.readyState === broadcasterSocket.OPEN) {
        // 通知廣播端有新觀眾加入，廣播端會主動發起連線邀請
        broadcasterSocket.send(JSON.stringify({ type: 'viewer-joined', viewerId: ws.id }))
      }
      return
    }

    // WebRTC 交換用的 SDP / ICE 轉發
    if (msg.type === 'offer' || msg.type === 'answer' || msg.type === 'ice-candidate') {
      const targetId = msg.targetId
      if (ws.role === 'broadcaster') {
        // 廣播端 → 指定觀眾
        const target = viewerSockets.get(targetId)
        if (target && target.readyState === target.OPEN) {
          target.send(JSON.stringify({ ...msg, fromBroadcaster: true }))
        }
      } else if (ws.role === 'viewer') {
        // 觀眾 → 廣播端
        if (broadcasterSocket && broadcasterSocket.readyState === broadcasterSocket.OPEN) {
          broadcasterSocket.send(JSON.stringify({ ...msg, viewerId: ws.id }))
        }
      }
      return
    }
    ...
    // 廣播端結束直播
    if (msg.type === 'broadcaster-end') {
      viewerSockets.forEach((vws) => {
        if (vws.readyState === vws.OPEN) {
          vws.send(JSON.stringify({ type: 'live-status', live: false }))
        }
      })
      return
    }
  ...
  ws.on('close', () => {
    if (ws.role === 'broadcaster' && broadcasterSocket === ws) {
    ...
    if (ws.role === 'viewer') {
      viewerSockets.delete(ws.id)
      if (broadcasterSocket && broadcasterSocket.readyState === broadcasterSocket.OPEN) {
        broadcasterSocket.send(JSON.stringify({ type: 'viewer-left', viewerId: ws.id }))
      }
    }
  })
})
